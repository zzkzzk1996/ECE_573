...
vector<vector<int>> FileReader::Reader(int dataset) {
    ...
    try {
        ...
        /* this part is for dataset choose */
        if (dataset == 0) {
            postFileName = postFileName0;
        } else if (dataset == 1) {
            postFileName = postFileName1;
        } else {
            throw "Error: Choose wrong dataset for wrong input !";  // since there's a wrong input
        }
